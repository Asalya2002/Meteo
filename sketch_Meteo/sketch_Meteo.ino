...
// Формат: T:25 H:40% P:755 (без дробных частей для компактности)
lcd.setCursor(0, 0);
...
lcd.setCursor(0, 1);
// Если уровень газа поднялся выше нормы на 80 единиц
if (gasVal > (air_clean_level + 80)) {
// --- РЕЖИМ ТРЕВОГИ ---
lcd.print("AIR: DANGER! ");
digitalWrite(PIN_BUZZER, HIGH); // Включаем звук
digitalWrite(PIN_RELAY, HIGH); // Включаем реле (например, вытяжку)
